...
slack_inbound_url = 'https://hooks.slack.com/services/T3S93LZK6/B3Y34B94M/inbound_link'

@application.route('/slack', methods=['POST'])
def inbound():
    delay = random.uniform(0, 10)
    time.sleep(delay)
    response = {'username': 'phoenix_bot', 'icon_emoji': ':computer:'}
    ...
    owner_name = 'phoenix'
    my_chatbot_name = 'phoenix_bot'
    if username in [owner_name, 'some.owner']:
        #response['text'] = text
        if text == "&lt;BOTS_RESPOND&gt;":
            ...
            response = {'username': 'phoenix_bot', 'icon_emoji': ':computer:', 'attachments': [{'color': '#36a64f', 'pretext': 'ANSWER 1','fields': [{'title': 'LINK', 'value': my_json['items'][0]['link'], 'short': False},{'title': 'Number of responses', 'value': str(my_json['items'][0]['answer_count']), 'short': True},{'title': 'DATE', 'value': str(datetime.datetime.fromtimestamp(my_json['items'][0]['creation_date'])), 'short': True}]}]}
            r = requests.post(slack_inbound_url, json=response)
            response = {'username': 'phoenix_bot', 'icon_emoji': ':computer:', 'attachments': [{'color': '#ad2d91', 'pretext': 'ANSWER 2','fields': [{'title': 'LINK', 'value': my_json['items'][1]['link'], 'short': False},{'title': 'Number of responses', 'value': str(my_json['items'][1]['answer_count']),'short': True}, {'title': 'DATE', 'value': str(datetime.datetime.fromtimestamp(my_json['items'][1]['creation_date'])), 'short': True}]}]}
            r=requests.post(slack_inbound_url, json=response)
            response = {'username': 'phoenix_bot', 'icon_emoji': ':computer:', 'attachments': [{'color': '#05afb2', 'pretext': 'ANSWER 3','fields': [{'title': 'LINK', 'value': my_json['items'][2]['link'], 'short': False},{'title': 'Number of responses', 'value': str(my_json['items'][2]['answer_count']),'short': True}, {'title': 'DATE', 'value': str(datetime.datetime.fromtimestamp(my_json['items'][2]['creation_date'])), 'short': True}]}]}
            r = requests.post(slack_inbound_url, json=response)
            response = {'username': 'phoenix_bot', 'icon_emoji': ':computer:', 'attachments': [{'color': '#3f00ff', 'pretext': 'ANSWER 4','fields': [{'title': 'LINK', 'value': my_json['items'][3]['link'], 'short': False},{'title': 'Number of responses', 'value': str(my_json['items'][3]['answer_count']),'short': True}, {'title': 'DATE', 'value': str(datetime.datetime.fromtimestamp(my_json['items'][3]['creation_date'])), 'short': True}]}]}
            r = requests.post(slack_inbound_url, json=response)
            response = {'username': 'phoenix_bot', 'icon_emoji': ':computer:', 'attachments': [{'color': '#e83006', 'pretext': 'ANSWER 5','fields': [{'title': 'LINK', 'value': my_json['items'][3]['link'], 'short': False},{'title': 'Number of responses', 'value': str(my_json['items'][4]['answer_count']),'short': True}, {'title': 'DATE', 'value': str(datetime.datetime.fromtimestamp(my_json['items'][4]['creation_date'])), 'short': True}]}]}
            r = requests.post(slack_inbound_url, json=response)
            ...
            topic=topic.strip()
            if topic.isdigit():
                weather_url = "http://api.openweathermap.org/data/2.5/weather?zip="+topic+"&units=imperial&APPID=private_id"
            else:
                geo_url = "http://www.datasciencetoolkit.org/street2coordinates/"+topic
                r=requests.get(geo_url)
                my_json = json.loads(r.text)
                weather_url ="http://api.openweathermap.org/data/2.5/weather?lat="+str(my_json[topic]['latitude'])+"&lon="+str(my_json[topic]['longitude'])+"&units=imperial&APPID=private_id"
            r = requests.get(weather_url)
            my_json = json.loads(r.text)
            response = {'username': 'phoenix_bot', 'icon_emoji': ':computer:','attachments': [{'color': '#36a64f','pretext':'WEATHER REPORT','fields': [{'title': 'Weather Description','value': my_json['weather'][0]['description'],'short': True},{'title': 'Humidity', 'value': str(my_json['main']['humidity'])+'%', 'short': True},{'title': 'Low', 'value': str(my_json['main']['temp_min'])+' Degree F', 'short': True},{'title': 'High', 'value': str(my_json['main']['temp_max'])+' Degree F', 'short': True},{'title': 'Wind Speed', 'value': str(my_json['wind']['speed'])+' MPH', 'short': True},{'title': 'Wind Angle', 'value': str(my_json['wind']['deg'])+' Degrees', 'short': True}]}]}
            r = requests.post(slack_inbound_url, json=response)
